      ...
      Using t As New Transaction(_s.FamilyDoc, "New Mass Family Document")
        If t.Start() Then
          ...
          Try
            ...
            Dim m_plane As Plane = _s.UiApp.Application.Create.NewPlane(New XYZ(0, 0, 1), New XYZ(0, 0, m_RmElev))
            ...
            For i = 0 To m_Boundary.Count - 1
              ...
              For ii = 0 To il.Count - 1
                ...
                Else
                  ...
                  If TypeOf m_Seg.Curve Is Line Then
                    ' New Curve
                    Dim m_cLine As Line = m_Seg.Curve.Clone
                    m_mc = _s.FamilyDoc.FamilyCreate.NewModelCurve(m_cLine, m_skplane)
                  End If
                  ...
                  If TypeOf m_Seg.Curve Is Arc Then
                    ' New Curve
                    Dim m_cLine As Arc = m_Seg.Curve.Clone
                    m_mc = _s.FamilyDoc.FamilyCreate.NewModelCurve(m_cLine, m_skplane)
                  End If
                  ...
                  If TypeOf m_Seg.Curve Is NurbSpline Then
                    ' New Curve
                    Dim m_cLine As NurbSpline = m_Seg.Curve.Clone
                    m_mc = _s.FamilyDoc.FamilyCreate.NewModelCurve(m_cLine, m_skplane)
                  End If
