...
#Region "Private Members"
  ...
  Private Function MakeLine(app As UIApplication, pLine As Line) As ModelCurve
    ...
    Try
      ...
      ' Plane & Sketch Plane
      Dim m_plane As Plane = app.Application.Create.NewPlane(m_norm, ptB)
  ...
  Private Function MakeArc(app As UIApplication, pArc As Arc) As ModelCurve
    ...
    Try
      ...
      Dim m_plane As Plane = app.Application.Create.NewPlane(ca)
      Dim m_skplane As SketchPlane = SketchPlane.Create(_doc, m_plane)
        ...
        Using t As New Transaction(_famDoc, "Transaction in Family Document")
          If t.Start() = TransactionStatus.Started Then
            ...
            Try
              ...
              ' Flat Sketchplane
              Dim m_plane As Plane = _uiApp.Application.Create.NewPlane(New XYZ(0, 0, 1), New XYZ(0, 0, m_RmElev))
              ...
              For i = 0 To m_Boundary.Count - 1
                ...
                For ii = 0 To il.Count - 1
                  ...
                  ' Curve is Line
                  If TypeOf m_Seg.Curve Is Line Then
                    Dim m_cLine As Line = TryCast(m_Seg.Curve, Line)
                    m_mc = _famDoc.FamilyCreate.NewModelCurve(m_cLine, m_skplane)
                  End If

                  ' Curve is Arc
                  If TypeOf m_Seg.Curve Is Arc Then
                    Dim m_cLine As Arc = TryCast(m_Seg.Curve, Arc)
                    m_mc = _famDoc.FamilyCreate.NewModelCurve(m_cLine, m_skplane)
                  End If

                  ' Curve is NurbSpline
                  If TypeOf m_Seg.Curve Is NurbSpline Then
                    Dim m_cLine As NurbSpline = TryCast(m_Seg.Curve, NurbSpline)
                    m_mc = _famDoc.FamilyCreate.NewModelCurve(m_cLine, m_skplane)
                  End If
